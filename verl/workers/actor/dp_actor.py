...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch_withconfidence(
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding and False:  # TODO confidence head does not support rmpad yet
                input_ids_rmpad, indices, cu_seqlens, *_ = unpad_input(
                    input_ids.unsqueeze(-1), attention_mask
                )  # input_ids_rmpad (total_nnz, ...)
                input_ids_rmpad = input_ids_rmpad.transpose(0, 1)  # (1, total_nnz)
            ...
            print("before output.confidence_scores.shape",output.confidence_scores.shape)
            if hasattr(output, "confidence_scores"):
    # output.confidence_scores shape: (batch_size,) 或 (batch_size, 1)
                confidence_scores = output.confidence_scores
                # 如果是 (batch_size, 1)，可以 squeeze(-1) 变成 (batsqqch_size,)
                # if confidence_scores.dim() == 2 and confidence_scores.size(-1) == 1:
                #     confidence_scores = confidence_scores.squeeze(-1)
            print("log_probs.shape",log_probs.shape)
            print("entropy.shape",entropy.shape if entropy is not None else None)
            print("confidence_scores.shape",confidence_scores.shape if confidence_scores is not None else None)
            ...
            # # 3. confidence 在这里算
            # last_hidden_state = full_hidden_states[:, -1, :]
            # confidence_scores = torch.sigmoid(self.actor_module.confidence_head(last_hidden_state))
            # print("11111111111111111111log_probs.shape",log_probs.shape)
            # print("11111111111111111111last_hidden_state.shape",last_hidden_state.shape)
            # print("11111111111111111111full_hidden_states.shape",full_hidden_states.shape)
            # print("1111111111111111entropy.shape",entropy.shape if entropy is not None else None)
            # print("11111111111111111confidence_scores.shape",confidence_scores.shape if confidence_scores is not None else None)
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        """
        # set to eval
        print("1234312312313231322432342_2")
    ...
    @GPUMemoryLogger(role="dp actor_withconfidence", logger=logger)
    def compute_log_prob_withconfidence(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        """Compute the log probability of the responses given input_ids, attention_mask and position_ids
    ...
    def update_policy(self, data: DataProto):
        ...
        if hasattr(data, "batch"):
            ...
            print("⚠️ data 没有 batch 属性")

        # 3️⃣ 打印 non_tensor_batch (非张量数据)
        if hasattr(data, "non_tensor_batch"):
            print("\n🧠 non_tensor_batch (non-tensor data):")
            for k, v in data.non_tensor_batch.items():
                if hasattr(v, "shape"):
                    ...
                    print(f"  {k}: type={type(v)}, value preview={preview}")
        else:
            print("⚠️ data 没有 non_tensor_batch 属性")
        ...
        if hasattr(data, "non_tensor_batch") and "extra_info" in data.non_tensor_batch:
            ...
            print("\n🧩 extra_info 详细内容：")
            print("shape:", getattr(extra_info, "shape", None))
            print("dtype:", getattr(extra_info, "dtype", None))
            # 尝试逐个打印
            try:
        ...
        if hasattr(data, "non_tensor_batch") and "reward_model" in data.non_tensor_batch:
            ...
            print("\n🧩 reward_model 详细内容：")
            print("shape:", getattr(reward_model, "shape", None))
            print("dtype:", getattr(reward_model, "dtype", None))
            # 尝试逐个打印
            try:
                for i, item in enumerate(reward_model):
                    print(f"  [{i}] → {item}")
            except Exception as e:
                print("⚠️ 无法逐项打印:", e)      
        ...
        if self.config.use_confidence_loss:
            if "rollout_preds" in data.non_tensor_batch.keys():
                ...
                print("adding rollout_preds to non tensor keys")
        data = data.select(batch_keys=select_keys, non_tensor_batch_keys=non_tensor_select_keys)
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    reward_model = model_inputs["reward_model"]
                    ground_truth = []
                    responses = model_inputs["responses"]   
                    print("reward_model",reward_model)
                    print("type of reward_model:", type(reward_model))
                    print("len of reward_model:", len(reward_model))
